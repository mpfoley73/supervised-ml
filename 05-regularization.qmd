...
$$\mathbb{B}(\hat{\beta}) = -\lambda \left(X'X + \lambda I \right)^{-1} \beta$$
...
<br>
A plot shows how the RMSE varies with the penalty. $R^2$ is shown too. The minimum RMSE is at `r select_best(ridge_resamples, metric = "rmse") |> pull(penalty) |> comma(.1)`.
...
The minimum RMSE is at `r select_best(lasso_resamples, metric = "rmse") |> pull(penalty) |> comma(.1)`.
...
In @eq-elnet-loss, new parameter $\alpha$ is a "mixing" parameter that balances the two approaches.  If $\alpha$ is zero, you are back to ridge regression, and if $\alpha$ is one, you are back to lasso. To fit a linear model with elastic net regression, specify `linear_reg(mixture = tune())`.
