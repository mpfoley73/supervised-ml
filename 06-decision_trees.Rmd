...
```{r}
oj_cart$tune %>% show_best("accuracy")
```
...
```{r}
oj_bag$tune %>% show_best("accuracy")
```
...
`collect_metrics()` returns the RMSE, $RMSE = \sqrt{(1/2) \sum{(actual - pred)^2}})$ and the model $R^2$. The RMSE of `r cs_bag$fit %>% collect_metrics() %>% filter(.metric == "rmse") %>% pull(.estimate) %>% comma(.01)` in the test data set is pretty good considering the standard deviation of `Sales` is `r comma(sd(cs_test$Sales), .01)`.
...
```{r}
oj_xgb$tune %>% show_best("accuracy")
```
